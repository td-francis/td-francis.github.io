...
ninja.data = [
  ...
  {%- for section in site.data.cv -%}
    {%- if section.type == "time_table" and section.contents -%}
      {%- case section.title -%}
        {%- when "Presentations" -%}
          {%- assign cv_target_url = "/presentations/" -%}
          {%- assign cv_use_anchor = false -%}
        {%- when "Teaching Experience (Instructor of Record)", "Teaching Experience (Teaching Assistant)" -%}
          {%- assign cv_target_url = "/teaching/" -%}
          {%- assign cv_use_anchor = false -%}
        {%- else -%}
          {%- assign cv_target_url = "/cv/" -%}
        ...
        {%- if cv_entry_title != blank -%}
          {%- assign cv_entry_desc = entry.institution | default: entry.department | default: entry.location -%}
          {
            ...
            handler: () => {
              ...
              {%- else -%}
                window.location.href = "{{ cv_target_url | relative_url }}";
              {%- endif -%}
            },
          },
        {%- endif -%}
